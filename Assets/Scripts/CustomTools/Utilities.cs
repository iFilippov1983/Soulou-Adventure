...
namespace CustomUtilities
{
    public static class Utilities 
    {
        ...
        /// <param name="baseSortingOrder"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Get direction the mouse points at
        /// </summary>
        ...
        /// <summary>
        /// Recognize if Mouse is over a UI Element? Used for ignoring World clicks through UI
        /// </summary>
        /// <returns></returns>
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static string Dec01ToHex(float value) => Dec_to_Hex((int)Mathf.Round(value * 255f));
        ...
        public static FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging) 
        {
            ...
            (
                ...
                if (dragging) {
                    onMouseDragging(Utilities.GetMouseWorldPosition());
                }
                return false; 
                }
            );
            ...
            ) 
        {
            ...
            return FunctionUpdater.Create
             (() => 
             {
                if (state == 1) 
                {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, Utilities.GetMouseWorldPosition());
                }
                ...
                if (Input.GetMouseButtonDown(mouseButton) && !Utilities.IsPointerOverUI()) 
                {
                    if (state == 0) 
                    {
                        state = 1;
                        from = Utilities.GetMouseWorldPosition();
                    } 
                    else 
                    {
                        state = 0;
                        onMouseClickFromTo(from, Utilities.GetMouseWorldPosition());
                    }
                }
                return false; 
             }
        ...
        public static void ShakeCamera(float intensity, float timer) 
        {
            ...
            FunctionUpdater.Create(delegate () 
            {
                ...
                Vector3 randomMovement = 
                    new Vector3(UnityEngine.Random.Range(-1f, 1f), UnityEngine.Random.Range(-1f, 1f)).normalized * intensity;
        ...
        /// <summary>
        /// Destroy all children and randomize their names, useful if you want to do a Find() after calling destroy, since they only really get destroyed at the end of the
        /// </summary>
        /// <param name="parent"></param>
        ...
        {
            int aimAngle = Utilities.GetAngleFromVector180(aimDir);
            int angle = Utilities.GetAngleFromVector180(posTarget - pos);
        ...
        public static List<Vector3> GetPositionListAround(Vector3 position, float distance, int positionCount) 
        {
            ...
            for (int i = 0; i < positionCount; i++) 
            {
                ...
                Vector3 dir = Utilities.ApplyRotationToVector(new Vector3(0, 1), angle);
        ...
        public static List<Vector3> GetPositionListAround
            (
            Vector3 position, 
            float distance, 
            int positionCount, 
            ...
            ) 
        {
            ...
            for (int i = 0; i < positionCount; i++) 
            {
                ...
                Vector3 dir = Utilities.ApplyRotationToVector(direction, angle);
                Vector3 pos = position + dir * distance;
                ret.Add(pos);
            }
            return ret;
        ...
        public static List<Vector3> GetPositionListAlongDirection
            (
            Vector3 position, 
            Vector3 direction, 
            float distancePerPosition, 
            int positionCount
            ) 
        {
            List<Vector3> ret = new List<Vector3>();
        ...
        public static List<Vector2Int> GetPosXYListOblong(int width, int dropXamount, int increaseDropXamount, Vector3 dir) 
        {
            ...
            list = RotatePosXYList(list, Utilities.GetAngleFromVector(dir));
            return list;
        }
        ...
        public static List<Vector2Int> RotatePosXYList(List<Vector2Int> list, int angle) 
        {
            ...
            for (int i = 0; i < list.Count; i++) 
            {
                ...
                Vector3 vec = Utilities.ApplyRotationToVector(new Vector3(posXY.x, posXY.y), angle);
        ...
        public static Transform CloneTransform
            (
            Transform transform, 
            string name, 
        ...
        public static Transform CloneTransformWorld
            (
            Transform transform, 
            Transform newParent, 
            string name, 
            ...
            ) 
        {
            ...
            {
                Vector3 vec1 = center + Utilities.ApplyRotationToVector(new Vector3(0, 1) * radius, (360f / divisions) * i);
                Vector3 vec2 = center + Utilities.ApplyRotationToVector(new Vector3(0, 1) * radius, (360f / divisions) * (i + 1));
                Debug.DrawLine(vec1, vec2, color, duration);
            }
        }

        public static void DebugDrawRectangle
            (
            Vector3 minXY, 
            Vector3 maxXY, 
            Color color, 
        ...
        public static void DebugDrawText
            (
            string text, 
            Vector3 position, 
            Color color, 
        ...
        public static void DebugDrawChar
            (
            char c, 
            Vector3 position, 
            Color color, 
        ...
        public static void DebugDrawLines
            (
            Vector3 position, 
            Color color, 
            float size, 
